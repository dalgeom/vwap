...
| 지표 | 목표 | 비고 |
|---|---|---|
| 승률 (distinct) | ≥ 55% | SMC v2와 같거나 높음 |
| 평균 승 / 평균 패 | ≥ 0.75 | SMC v2 (0.87) 수준 |
| 거래당 EV | ≥ +0.15% | SMC v2 (0.08%) 2배 이상 |
| TP1 도달률 | ≥ 30% | SMC v2 (25%) 상회 |
| TIMEOUT 비율 | ≤ 20% | 구조적 문제 없음 |
| 주 거래 빈도 | 5~10건 | 빈도 목표 달성 (일 평균 1~2건) |
| 프로핏 팩터 | ≥ 1.3 | 지속 가능성 지표 |
| 최대 드로우다운 | ≤ 15% | 심리적 견딜 수 있음 |
...
> 이전 합의 없음 상태 해소. Agent G 검토 → 긴급 재회의에서 Agent F(윤세영) 확정.
> **2026-04-20 회의 #15 보강**: `va_slope` 의 계산 공식이 본 기획서에서 누락되어 있었던 점을 정정. 공식은 회의 #2 에서 이미 확정된 것을 부록 H-1.2 에 정식 수록. (Agent G Critical 6, Agent E 책임)
...
def compute_trailing_sl_module_b(
    ...
    if direction == "long":
        ...
        chandelier_sl = new_extreme + CHANDELIER_MULT * atr_1h
        new_trailing_sl = min(chandelier_sl, prev_state.trailing_sl)  # ← initial_sl 제거 (Agent G 버그 수정)
        new_state = "TRAILING" if new_trailing_sl < initial_sl else "INITIAL"
...
class RiskManager:
    ...
    # ✅ 모듈별 독립 카운터 + 시스템 합산 카운터 (Agent G 버그 수정)
    ...
    def can_enter(
        ...
        # ✅ 모듈별 1개 제한 (Agent G Critical 3 수정)
    ...
    def get_position_size_pct(self) -> float:
        """동시 2포지션 시 리스크 0.75%로 축소 (Agent G Major 10 수정)"""
...
> **확정**: 긴급 재회의 (Agent G Major 9 수정), Agent F 결정
...
> **확정**: 긴급 재회의 (Agent G Critical 2 수정), Agent F 결정
...
> **정식 수록**: 2026-04-20 회의 #15 — 기존 누락 정정 (Agent G Critical 6).
...
> **확정**: 긴급 재회의 (Agent G Critical 4 수정), Agent F 결정
...
def compute_position_size(
    ...
    risk_pct: float = 0.02, # ✅ Agent G N2 수정: RiskManager.get_position_size_pct() × 0.02
...
| vwap_sigma_entry 범위 | 고정 -2.0 (Grid 제외) | ✅ 회의 #16 (A 옵션 1, 2026-04-21) |
| Long (i) σ 척도 (Module A) | ATR(14), close 기준, 배수 -2.0 | ✅ 회의 #18 (결정 #19, 2026-04-21) |
...
| Tier 1 왕복 비용 | Module A 0.10%, Module B 0.16% | ✅ Agent F 확정 |
| Tier 2 왕복 비용 | Module A 0.16%, Module B 0.24% | ✅ Agent F 확정 |
