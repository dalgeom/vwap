...
> 상태: **⛔ NO-GO 기각 (2026-07-21 실행)** — 16조합 전부 net −(0.86~1.11%)·양수 0%. 체결거래 gross −0.78%(taker +0.13%에서 음수 전락) = passive maker가 빨리-되돌림 승자 놓치고 안-되돌림 패자만 체결(adverse selection). 수수료 절감(net-gross差 ~0.08%)은 gross가 이미 음수라 무의미. taker(엣지<비용)+maker(adverse selection) → pairs 두 실행모드 모두 사망, 완전 종료. 이력 §10.
