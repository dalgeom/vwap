...
### 5.7 163건 중간점검 + v7 라이브 현황 (2026-06-17, 타 PC 재가동 후)

> ★ **정본기반 누적 +$3,417** (corrected106 +$1,389 + 신규57 +$2,028). ⚠️ **live 원본 jsonl 합은 +$1,352 = 오염**(과거 PnL 버그 GRASS2 −1798 등 미반영) → 분석은 정본기반, raw 누적 쓰지 말 것. 신규분 다수 estimated(거래소 미정정).

- **★ 손절 89건 분해 (신규 정량)**: 즉시역행(MFE<0.5%, 진입 직후 반대) **52건 −$7,033 = 손절 손실의 67%** / 한때 떴다 꺾임(MFE≥0.5%) 37건 −$3,467. hold≤2 즉사 56건 / ≥3 33건. → **손실의 3분의 2가 입구 즉시역행 = 진입시점 변별 불가**(H1·§8.1·§8.5 강화). 출구개선(BE 당김)으로 구제 가능한 건 "떴다 꺾인 37건"뿐, 즉시역행 52건은 입구·사이징 외엔 손 못 댐. → **목표는 "손절 건수↓"가 아니라 "건당 EV↑"가 정확**(손절은 작게 끊는 안전장치, 건당 −$100대·catastrophic 부재).
- **방향(raw, 부호는 robust)**: short 58건 EV+$28.6 PF1.60 vs long 105건 EV−$2.9 PF0.96. short 우위 지속(v7 정원확장 정합). 단 잭팟 top5(VELVET+1386·BEAT+894·NEAR+728·BEAT+685·PORTAL+649 = **+$4,341**)는 **전부 long** → top5 빼면 158건 **−$924**. 흑자=소수 long 잭팟 의존 재확인(설계특성).
- **★ v7 라이브 = 사실상 미검증**: 재시작(06-15 04:24Z) 후 *진입* 청산 5건(EVAA−141·BEAT−127·XRP+116·ZRO+10·FART−1) **net −$144, n5** + SPX 보유. **정원확장 0발동**(저변동 가뭄에 기본 3자리도 안 차 4·5번째 미도달) = v7 핵심기능 시험조차 안 됨. 거래량 BEAT `vol_ratio` 7.5(첫 큰 값, 평소 7.5배 폭발)→즉사 손절(n1, 결론 없음, 기록 개시).
- ★ **OPG/ZKC 잭팟은 v6 진입분**(06-15 02:00, 재시작 전): OPG short 단발(consec0) Timeout **+$409**(+42%)·ZKC short 단발 Timeout +$81. → **단발 short가 만기 잭팟 = v7 short 처방(확장자리=consec≥1 꾸준만)의 forward 반례 2건**(H14). `track_cap.py` 누적 관찰, n 소·peeking 금지.
- **방법론 재정립(사용자 합의)**: 목적=수많은 factor 조합으로 우상향 edge 탐색이나 **진짜 제약은 factor 부족이 아니라 표본 부족**(과최적화/데이터마이닝 함정). 163건에 factor 남발 = 가짜 패턴 양산 → **§11 사전등록 + 200건 일괄검정이 정공법**. 지금 단계 미덕 = factor 추가가 아니라 표본 채울 때까지 참기.

### 5.8 180건 인계 점검 (2026-06-22, PC 이전) — arm B 첫 잭팟

> 163→180건(+17). 정본기반 누적 ≈ +$3,4xx(corrected106 + 신규). 인계 시점 **무포지션·equity ~$25,686·봇 일시중단**(STOP_MOMENTUM). 06-18~19 변동성 잠깐 복귀(잭팟 다발) 후 다시 가뭄(매시간 34코인 scan, 0 candidates·shadow0 = 막힌 게 아니라 큰 파도 자체 부재).

- **★★ arm B(빠른 본전잠금 0.75) 첫 잭팟 = ESPORTS short +$283**(거래량 4.87배·−73% 폭락·Timeout, position_size $385 소액). §5.6 "arm B 잭팟 0건 판정불가"가 **깨짐** → "빠른BE가 대박을 일찍 잘라먹는다"는 우려의 첫 반례(n=1, 단정 금지, arm별 BE 재검토 신호).
- **거래량(`signal_vol_ratio`) 첫 양면 데이터**: BEAT 7.5배 폭발→즉사 손절 / ESPORTS 4.87배→+$283 잭팟. **단독신호 아님 재확인**(다른 조건과 조합돼야, §8.5 정합).
