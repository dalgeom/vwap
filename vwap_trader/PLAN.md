...
| 2026-07-08 | **★ Step2 완료(반사실 계측기 배포) + 리포트 개편·AI 성찰 + Step3 사전등록(§11.1)**: 계측기 `be_counterfactual.py`(순수) + 봇 3배선(진입초기화·trailing끝 그림자갱신·log_trade 쌍기록), 실매매 무변경·try격리·토글, 재가동 배포(레거시 4포지션은 shadow 없음=스킵, 새 진입부터 쌍 수집). 일일 리포트 1인칭 봇 보고서로 개편(보유·청산·계측기 A/B·걸른신호 사실만·자아성찰) + **AI 성찰**을 `claude -p`(구독 headless=무과금)로 자동 생성(wrapper, 인코딩 파일리다이렉션 견고화). Step3: I3(BE A/B) 판정 §11.1 봉인(100쌍·Wilcoxon·잭팟 거부권·EV 미적용). |
| 2026-07-08 | **★ v10 게이트 점검(40건) + 슬리피지 오경보 정정 + C-1 보류**: **v10 개선 증거 0** — EV+$86.5은 2종목 잭팟 5건(TAIKO 총+2,687·LAB+2,078)이 전부, **top5 제외 EV−$42·net−$1,471**(pre-v10 top5제외 −$6.9보다 나쁨)·최대승자 2개만 빼도 본전. 롤백근거도 없음(잭팟변동 지배·소표본) → **무변경·수집지속, "v10 순항" 서사 폐기.** **★ 슬리피지 "5배 증가"=오경보(정정)**: fill==intended 비율 100%(5월)→0%(6/4 §8.7 entry_price 수정 경계)=초기엔 체결가를 신호가로 기록(가짜0), 수정 후 진짜 체결 측정. 소형알트만 6/5이후 시계열 0.17~0.21% **평평(안 커짐)**. 진짜 상시비용 소형알트 median~0.2%·mean~0.4%(꼬리 2.7%)=늘 있던 것, 실행악화 아님. **★ C-1(소형코인 상한완화) 보류**: 잭팟이 tier4 $1000에 묶인 소형알트라 방향은 맞으나(TAIKO+1186이 $1000베팅), **비잭팟 base 마이너스 + 소형알트 슬리피지 실재** → 사이즈업=손실·슬리피지도 2배, 잭팟우위 검증불가(되감기 폐기)=도박. 데모라 실슬리피지 더 나쁨. 무변경 |
| 2026-07-09 | **★ 전문가 재점검 반영: 유일본 push + §11.1 개정 + whipsaw 기각**: ① **미푸시 28커밋(오늘 전작업) origin push**(§7-4 유일본 해소=최우선 리스크). ② **§11.1 BE A/B 판정 1회 개정·재봉인**(쌍0개=유일 개정창): 검증결과 분기구간 [0.75,1.5)ATR **11.9%뿐**(정본243, 전문가12.6% 일치)→총100쌍 Wilcoxon 기여~12쌍=판정불가. 게이트를 **분기쌍30개**(검정력~89%)+상한(16주/350쌍), 잭팟=arm불변 R≥7.8(top5컷 실측), **1차=잭팟포함 ΣR차이**/Wilcoxon 2차강등, 거부권 순손익규칙(별항). ③ **whipsaw 쿨다운 기각(기저율 검증)**: 자아성찰 일화(BLUR 양방향 손절, n=2)를 정본243 기저율로 검증 → SL후24h 재진입 40건 avg+$25(비재진입+$27 유사), **같은방향 20건 +$64**(잭팟 PORTAL+649·H+568·HYPE+373 포함)=쿨다운 시 절단. 반대방향만 20건 −$14(사후하위군·소표본). → 쿨다운 기각, **반대방향 재진입만 관찰 플래그**(승격: 40건 도달 시 1회 평가·여전히 음수&잭팟0일 때만). **★사이클 실증: 일화→기저율→기각으로 잭팟 절단 실수 방지.** ★★ 전문가 진단 프로세스 4처방(배포≠검증완료·합성주입테스트·생존카운터·계측 diff리뷰) 채택 예정 |
...
### 11.1 I3(BE A/B) 반사실 계측 판정 — 사전등록 (2026-07-08 최초, 2026-07-09 1회 개정·재봉인)

**★ 개정 사유(쌍 0개 시점의 유일한 정당한 개정, 전문가 통계심사)**: 최초 "총 쌍 100개" 게이트가 검정력 결함. 실측(정본 243건 MFE÷ATR): MFE<0.75ATR **45.7%**(두 arm 미발동=완전동률)·MFE≥1.5ATR **42.4%**(두 arm 본전=대략동률)·**실제 갈리는 [0.75,1.5)ATR 구간 11.9%뿐**. Wilcoxon은 동률 쌍을 버려 총 100쌍→기여 ~12쌍→큰 효과도 검출력 부족(판정일 거의 확실히 판정불가). 아래로 개정.

계측기(`data/be_counterfactual.jsonl`)가 매 거래 실제 arm+반대 arm(그림자)을 편향0 기록. arm A(1.5 느린잠금) vs B(0.75 빠른잠금). **§11 기본규율 "주지표=EV" 미적용**(잭팟지배).

- **판정 게이트(봉인)**: **분기 쌍 30개**(두 arm 결과가 다른 쌍) 도달 시 **1회** 판정(검정력 ~89%). 총 쌍 환산 ~240(현 페이스 7~10주). **상한**: 16주 경과 또는 총 350쌍 중 먼저 오는 시점 데이터로 강제 판정. 중간 **결과 열람** 금지.
- **분기/동률 정의**: 봉 근사 아님 — 계측기의 **arm별 트리거·청산 타임스탬프**로 정의(두 arm 청산가/사유가 다르면 분기).
- **잭팟 정의(arm-불변 절대기준)**: 쌍별 `max(pnl_A,pnl_B)`를 R로 정규화(R=pnl÷[1.5×ATR÷entry×size]$ 리스크), **R ≥ 7.8**(정본 223건 top5 컷라인 실측=7.83R 상수 고정). ※ 순위기준 "top5 제외"는 컷라인이 판정시 움직여 조작여지 → 폐기.
- **1차 지표(경제 실질)**: **잭팟 포함 ΣR 차이**(ΣR_B − ΣR_A > 0). 방향 단측 사전지정. — 잭팟 제외 Wilcoxon은 분기 SL 쌍에서 기계적으로 B 우세(B 본전/A 풀손절)라 1차로 두면 착시.
- **2차 지표(메커니즘 확인)**: 잭팟(R≥7.8) 제외 분기 쌍의 쌍별 차이 `B−A` Wilcoxon signed-rank(단측). 1차와 방향 일치 확인용(강등).
- **잭팟 거부권(1순위)**: 분기 쌍 중 `A≥5R 이고 B≤0.5R`(B가 잭팟을 이른 본전이탈로 절단) 발생 시 — **순손익 규칙**: 훼손 손실 R(Σ(A−B), 해당 쌍) > B가 손절구간에서 아낀 R(Σ(B−A), 손실 분기쌍)이면 **B 기각**. 애매하면 보수적 **판정 유보 + 분기 쌍 30개 추가**.
- **판정 처리**: 1차(ΣR 차이 +) AND 거부권 통과 → **B 채택 후보**(다음 단일 변경, forward 재확인). 아니면 A 유지·I3 종결. §10 기록.
- **분기 쌍 카운트 = peeking 아님**(결과 미열람, outcome-blind) → 일일 리포트에 분기쌍 N·생존 카운터·마지막 쌍 시각 표기 **허용**.
- **한계**: 그림자 청산가=shadow_sl 가정, 2차효과 미포함. real_exchange_pnl vs pnl_of 대조로 계측 신뢰도 상시 점검.
