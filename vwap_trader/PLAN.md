...
## 5. 운영 현황 스냅샷 (§5.0~5.3은 2026-06-05/106 trades 시점, **최신 누적은 §5.12 — 274건**)
...
- **처방(다음 세션 1순위, 미구현 — 순서대로)**: ① **결함③ 먼저**(가장 싸고 가장 위험) — `update_shadow`를 봇과 동일하게: 추적선은 `be` 발동 후에만, spike-retrace 가드도 `be` 조건부. **봇 `_update_trailing_sl`과 그림자가 같은 함수를 쓰도록 통합**하는 게 근본 처방(정책이 갈리는 구조 자체를 제거). ② 결함② — `_log_trade` 강제마감 전, **실제 청산가가 그림자 SL 너머면 그림자는 shadow_sl에서 이탈**로 확정(단순·확정적). ③ 결함① — real 청산 후에도 그림자를 **캔들만으로 계속 추적**(유령 포지션, 거래소 미접촉, 자체 스탑 걸릴 때까지). ⚠️ 이건 되감기 백테스트가 아니라 **forward 실시간 추적**이라 §8.9+ 강등 대상 아님. ④ 수리 후 **기존 29쌍 폐기·카운터 리셋**(계측기가 고장이었으므로. **§11.1 판정기준 자체는 손대지 않음** — 결과를 보고 고치는 게 아니라 계량기를 고치는 것이므로 사전등록 무결성 유지). ⑤ **합성 주입 테스트 필수**(전문가 처방 ②): 인위적 분기 시나리오를 넣어 분기가 실제로 기록되는지 확인한 뒤에야 배포 — 이번 사고의 재발 방지책.
...
- ★ **상위5 제외 net −$947.29(EV −$35)** = 이 기간 수익 전부가 잭팟 5건. **잭팟 의존 구조 불변 재확인**: LAB short Timeout **+$1,529**·ESPORTS long TrailSL +$596·EVAA long TrailSL +$539·B short TrailSL +$324·ESPORTS long TrailSL +$264.
...
- **v10 구간**: 49건 → **78건** / net +$5,767 / EV +$73.94. ★ **상위5 제외 net −$654(EV −$8.96)** — 07-08 게이트 점검 시점(40건, 상위5제외 −$1,471)보다 **비잭팟 기저가 +$817 개선**. 단 잭팟 구성이 바뀐 소표본 비교라 **v10 개선 증거로 쓰지 말 것**(07-08 "v10 개선 증거 0" 판정 유지).
