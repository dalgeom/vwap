...
### 11.1 I3(BE A/B) 반사실 계측 판정 — 사전등록 (2026-07-08, 봉인)

전문가 자문(§10 2026-07-08) 반영. 되감기 폐기 후 **봇 내장 반사실 계측기**(Step 2, `data/be_counterfactual.jsonl`)가 매 거래 1쌍(실제 arm + 반대 arm 그림자)을 편향 0으로 기록 → 이 데이터로 I3(BE 트리거 A=1.5 느린잠금 vs B=0.75 빠른잠금)을 판정. **§11 기본규율의 "주지표=EV"는 이 실험엔 적용 안 함** — 손익 잭팟지배로 EV 검정 불가. 대신:

- **판정 시점(봉인)**: 쌍 **100개** 도달 시 **1회** 판정. 중간 peeking으로 조기 적용/종료 금지(현 빈도 ~4/일 → ≈3~4주).
- **1차 지표(손실 엔진, 통계 가능)**: top5 잭팟(real_pnl 상위 5쌍) **제외** 후, 쌍별 차이 `B_pnl − A_pnl`을 **Wilcoxon signed-rank**(이상치 강건)로 검정. 방향 단측 사전지정 = "B가 손실쪽을 줄인다(중앙값>0)". 통과 = 부호(+) AND p<0.05.
- **잭팟 거부권(1순위)**: top5 잭팟 쌍 중 B가 잭팟을 명백히 훼손(`B_pnl < A_pnl` 이고 차이 > 1R=1.5×ATR×사이즈)한 게 **1건이라도** 있으면 1차 통과해도 **B 기각**(빠른 본전이탈로 잭팟 절단이 핵심 위험).
- **결과 처리**: 1차 통과 + 거부권 통과 → **B 채택 후보**(다음 단일 변경으로 실제 arm 변경 검토, forward 재확인). 아니면 **A(현행) 유지·I3 종결**. 어느 쪽이든 §10 기록.
- **계기판(판정 아님, 참고만)**: 잭팟 빈도·MFE 캡처율·반납률 — 매일 리포트 계측기 섹션에 방향만, p값 없음.
- **한계**: 그림자 청산가=shadow_sl 가정, 2차효과(자리·자본 재사용) 미포함 → 방향성 판정(계측기 스펙 2026-07-08). real_exchange_pnl vs pnl_of(real_exit) 대조로 계측 자체 신뢰도 상시 점검.
