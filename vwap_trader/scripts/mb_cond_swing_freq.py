"""
TASK-MB-008: Module B Long — 스윙 구조 기반 신규 조건 동시 성립 빈도 확인

  Cond A: close > VWAP_daily  AND  EMA9_1h > EMA20_1h
  Cond B: 4H EMA9 > 4H EMA20  (직전 확정 4H 봉 기준, 룩어헤드 금지)
  Cond C: 스윙 구조 되돌림 38~62%
           H_swing = 현재 봉 기준 ±SWING_N 봉 윈도우 최고 고점
  ...
  H_swing/L_swing 탐색: spec 정의 "좌우 5봉" — ±5봉 대칭 윈도우 사용 (빈도 분석용).
"""
...
def main() -> None:
    ...
    out_path = RESULT_DIR / f"mb_cond_swing_freq_{ts_str}.json"
    ...
    print()
    print("=" * 70)
    print("TASK-MB-008: Module B Long -- swing struct cond freq check")
    ...
    note_parts = [
        "H_swing: ±5봉 윈도우 최고 고점.",
        "L_swing: H_swing 이전 ±5봉 로컬 미니멈.",
    ...
    if fail_symbols or warn_symbols:
        ...
        print("=" * 70)
        if fail_symbols:
            print("[FAIL] P&L 착수 금지. 즉시 의장 보고 요망.")
            print(f"   FAIL symbols: {', '.join(fail_symbols)}")
        if warn_symbols:
            print("[WARN] 즉시 의장 보고 (P&L 착수 후 F 통보).")
