...
def ensure_cache(symbol: str) -> Path:
    path = CACHE_DIR / f"{symbol}_{INTERVAL}.csv"
    if path.exists():
        print(f"  {symbol}: 캐시 있음 ({path.name})")
        return path
    print(f"  {symbol}: 수집 중...")
    ...
    print(f"    저장: {path.name}  행수: {len(rows)}  {first_dt.date()} ~ {last_dt.date()}")
    return path
...
def _trade(trades: list, entry: float, exit_p: float,
           atr_sig: float, reason: str, hold: int,
    ...
    for i in range(n1):
        ...
        if dt < START_DT or dt > END_DT:
            if in_pos and dt > END_DT:
                _trade(trades, e_price, r1["open"], atr_sig, "PERIOD_END",
                       i - e_idx, e_dt, dt)
                in_pos = False
            continue
        ...
        if in_pos and i > e_idx:
            ...
            if ep is not None:
                _trade(trades, e_price, ep, atr_sig, er, i - e_idx, e_dt, dt_exit)
                in_pos = False
...
def main() -> None:
    ...
    print("결정 #45: AVAX/LINK 편입 검증 백테스트")
    ...
    for sym in NEW_SYMBOLS:
        sd = precompute(sym)
        print(f"  {sym}: 1H={sd['n1']}봉  4H={sd['n4']}봉")
    ...
    for sym in NEW_SYMBOLS:
        r = results[sym]
        ev_ok   = r["ev"] > 0
        freq_ok = r["daily"] >= 0.05
        verdict = "편입" if (ev_ok and freq_ok) else "제외"
        verdicts[sym] = verdict
        sign = "+" if r["ev"] >= 0 else ""
        print(f"\n심볼: {sym}")
        print(f"건수: {r['n']}  건/일: {r['daily']:.3f}  승률: {r['wr']*100:.1f}%  "
              f"PF: {r['pf']:.3f}  EV: {sign}{r['ev']*100:.3f}%")
        print(f"판정: {verdict}")

    print()
    included = [sym for sym in NEW_SYMBOLS if verdicts[sym] == "편입"]
    avax_d = results["AVAXUSDT"]["daily"] if "AVAXUSDT" in included else 0.0
    link_d = results["LINKUSDT"]["daily"] if "LINKUSDT" in included else 0.0

    total_daily = BTC_DAILY + SOL_DAILY + avax_d + link_d
    target_met  = total_daily >= 0.555

    parts = [f"BTC: {BTC_DAILY:.3f}", f"SOL: {SOL_DAILY:.3f}"]
    if avax_d > 0:
        parts.append(f"AVAX: {avax_d:.3f}")
    if link_d > 0:
        parts.append(f"LINK: {link_d:.3f}")

    print("[합산 빈도 -- 편입 심볼 전체]")
    print(" + ".join(parts) + f" = {total_daily:.3f}건/일")
    print(f"철칙 달성 (≥0.555건/일): {'YES' if target_met else 'NO'}")

    # JSON 저장
    out_path = RESULT_DIR / f"bt013_avax_link_{ts_str}.json"
    output = {
        "task":    "BT-013",
        "run_at":  now.strftime("%Y-%m-%dT%H:%M:%SZ"),
        "strategy": "4H Swing Breakout + 1H Pullback Long — AVAX/LINK 편입 검증",
        ...
        "trades": {sym: results[sym]["trades"] for sym in NEW_SYMBOLS},
    }
