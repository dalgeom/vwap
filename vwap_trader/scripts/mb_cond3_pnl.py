"""
TASK-MB-005: Module B Long -- Cond1+2+3 P&L 검증 (MB-003 직접 비교)
  Cond 1: close > VWAP_daily  AND  EMA9_1h > EMA20_1h
  Cond 2: abs(close - EMA9_1h) <= 0.5 x ATR_14_1h
  Cond 3: close > open  (양봉 반전 확인)  <-- MB-003 대비 추가

  SL / TP / max_hold / 비용 모델: MB-003과 완전 동일 -- 변경 금지
"""
...
def calc_indicators(rows: list[dict]) -> list[dict]:
    ...
    for i, row in enumerate(rows):
        result.append({**row, "ema9": ema9_series[i], "ema20": ema20_series[i], "atr14": atr14_series[i]})
    return result
...
def simulate_trades(rows: list[dict]) -> list[dict]:
    ...
    for i, row in enumerate(rows):
        ...
        if not in_position:
            ...
            cond3 = row["close"] > row["open"]   # 양봉

            if cond1 and cond2 and cond3 and atr14 > 0:
                next_idx = i + 1
...
def main() -> None:
    ...
    out_path   = RESULT_DIR / f"mb_cond3_pnl_{ts}.json"
    trade_path = RESULT_DIR / f"mb_cond3_pnl_{ts}_trades.json"
    ...
    print()
    print("=" * 70)
    print("TASK-MB-005: Module B Cond1+2+3  P&L  (vs MB-003)")
    print(f"  SL={SL_MULT}xATR  TP={TP_MULT}xATR  max_hold={MAX_HOLD_BARS}bars  cost=0.14% RT")
    print("=" * 70)

    headers = ["item", "BTC MB-003", "BTC MB-005", "ETH MB-003", "ETH MB-005"]
    rows_tbl = [
        ("daily_avg",      MB003["BTCUSDT"]["daily_avg"],       sym_results["BTCUSDT"]["daily_avg"],
                           MB003["ETHUSDT"]["daily_avg"],       sym_results["ETHUSDT"]["daily_avg"]),
        ("win_rate_%",     MB003["BTCUSDT"]["win_rate_pct"],    sym_results["BTCUSDT"]["win_rate_pct"],
                           MB003["ETHUSDT"]["win_rate_pct"],    sym_results["ETHUSDT"]["win_rate_pct"]),
        ("avg_win_atr",    MB003["BTCUSDT"]["avg_win_atr"],     sym_results["BTCUSDT"]["avg_win_atr"],
                           MB003["ETHUSDT"]["avg_win_atr"],     sym_results["ETHUSDT"]["avg_win_atr"]),
        ("avg_loss_atr",   MB003["BTCUSDT"]["avg_loss_atr"],    sym_results["BTCUSDT"]["avg_loss_atr"],
                           MB003["ETHUSDT"]["avg_loss_atr"],    sym_results["ETHUSDT"]["avg_loss_atr"]),
        ("EV/trade_atr",   MB003["BTCUSDT"]["ev_per_trade_atr"],sym_results["BTCUSDT"]["ev_per_trade_atr"],
                           MB003["ETHUSDT"]["ev_per_trade_atr"],sym_results["ETHUSDT"]["ev_per_trade_atr"]),
        ("profit_factor",  MB003["BTCUSDT"]["profit_factor"],   sym_results["BTCUSDT"]["profit_factor"],
                           MB003["ETHUSDT"]["profit_factor"],   sym_results["ETHUSDT"]["profit_factor"]),
        ("MDD_%",          MB003["BTCUSDT"]["mdd_pct"],         sym_results["BTCUSDT"]["mdd_pct"],
                           MB003["ETHUSDT"]["mdd_pct"],         sym_results["ETHUSDT"]["mdd_pct"]),
        ("SL_rate_%",      MB003["BTCUSDT"]["sl_rate_pct"],     sym_results["BTCUSDT"]["sl_rate_pct"],
                           MB003["ETHUSDT"]["sl_rate_pct"],     sym_results["ETHUSDT"]["sl_rate_pct"]),
        ("TP_rate_%",      MB003["BTCUSDT"]["tp_rate_pct"],     sym_results["BTCUSDT"]["tp_rate_pct"],
                           MB003["ETHUSDT"]["tp_rate_pct"],     sym_results["ETHUSDT"]["tp_rate_pct"]),
        ("timeout_%",      MB003["BTCUSDT"]["timeout_rate_pct"],sym_results["BTCUSDT"]["timeout_rate_pct"],
                           MB003["ETHUSDT"]["timeout_rate_pct"],sym_results["ETHUSDT"]["timeout_rate_pct"]),
    ]

    print(f"\n  {'item':<18} {'BTC-003':>10} {'BTC-005':>10} {'ETH-003':>10} {'ETH-005':>10}")
    print(f"  {'-'*18} {'-'*10} {'-'*10} {'-'*10} {'-'*10}")
    for row in rows_tbl:
        print(f"  {row[0]:<18} {row[1]:>10} {row[2]:>10} {row[3]:>10} {row[4]:>10}")

    print(f"\n  by_year (BTC):")
    for yr, ys in sym_results["BTCUSDT"]["by_year"].items():
        print(f"    {yr}: trades={ys['total_trades']}  win={ys['win_rate_pct']}%  EV={ys['ev_per_trade_atr']}  PF={ys['profit_factor']}")

    print(f"\n  by_year (ETH):")
    for yr, ys in sym_results["ETHUSDT"]["by_year"].items():
        print(f"    {yr}: trades={ys['total_trades']}  win={ys['win_rate_pct']}%  EV={ys['ev_per_trade_atr']}  PF={ys['profit_factor']}")
    ...
    output = {
        ...
        "verdict": verdict,
        "note": (
            f"Cond3(close>open) 추가 결과. BTC EV={btc_ev:.4f}. "
            f"EV improved vs MB-003: {vs['ev_improved']}. "
            f"SL rate improved: {vs['sl_rate_improved']}. "
            f"EV_NEGATIVE -> awaiting chairman instruction."
            if verdict == "EV_NEGATIVE" else
            f"Cond3(close>open) 추가 결과. BTC EV={btc_ev:.4f}. EV_POSITIVE."
        ),
    }
