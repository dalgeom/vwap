"""
TASK-SIMPLE-005: Condition 1 + 양봉 필터 P&L 검증
entry : close < VWAP_daily - 1.0 x ATR_14_1h  AND  close > open  (양봉)
TP    : close >= VWAP_daily
SL    : close <= entry_price - 1.5 x ATR_entry
...
ATR_PERIOD     = 14
ENTRY_SIGMA    = 1.0   # close < VWAP - 1.0 x ATR
SL_SIGMA       = 1.5   # SL = entry - 1.5 x ATR
TIMEOUT_BARS   = 8
...
def run_backtest(symbol: str, rows: list[dict]) -> dict:
    ...
    for i in range(n):
        ...
        if not in_position:
            # 진입 조건: Condition 1 AND 양봉
            if close < vwap - ENTRY_SIGMA * atr and close > rows[i]["open"]:
                in_position   = True
                entry_price   = close            # 신호 캔들 close 진입
                entry_atr     = atr
            ...
            # TP: close >= VWAP_daily (현재 날의 누적 VWAP)
            if close >= vwap:
                exit_price  = close
                exit_reason = "TP"
            # SL: close <= entry - 1.5 x ATR_entry
            elif close <= sl_price:
                exit_price  = close
                exit_reason = "SL"
            # Timeout: 8봉 경과
...
def main() -> None:
    ...
    out_path = RESULT_DIR / f"simple_cond1_bullish_{ts}.json"
    ...
    print()
    print("=" * 62)
    print("TASK-SIMPLE-005: Condition 1 + bullish candle filter")
    print(f"  TP: close >= VWAP_daily")
