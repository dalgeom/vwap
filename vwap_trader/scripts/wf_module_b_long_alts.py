"""
Walk-Forward Validation -- Module B Long (ETH / SOL / BNB)
PLAN.md SS L.5 / 결정 #42 확정 파라미터

판정 기준:
  WF 효율 = mean(OOS scores) / mean(IS scores) >= 0.70
  AND 전체 OOS EV > 0

알트 MDD 허용: <= 25% (결정 #40)
스코어 = PF x win_rate / max(MDD, 0.001)
"""
...
CHANDELIER_MULT = 3.0
MAX_HOLD_BARS   = 72
ROUND_TRIP_FEE  = 0.0007   # 편도 fee+slip (tier1 기준)

MDD_LIMIT_ALT   = 0.25     # 결정 #40

# -- Walk-Forward 설정 (PLAN.md SS L.5, 결정 #16) -----------------------
    ...
    for i, r in enumerate(rows):
        ...
        if dt < start_dt or dt > end_dt:
            if in_position and dt > end_dt:
                ...
                trades.append({"pnl_pct": pnl_pct, "pnl_atr": pnl_atr, "reason": "PERIOD_END"})
                in_position = False
            continue
        ...
        if in_position and i > entry_idx:
            ...
            if exit_price is not None:
                ...
                pnl_pct   = (eff_exit - eff_entry) / entry_price
                pnl_atr   = (eff_exit - eff_entry) / atr_signal if atr_signal > 0 else 0.0
                trades.append({"pnl_pct": pnl_pct, "pnl_atr": pnl_atr, "reason": exit_reason})
                in_position = False
        ...
        # 진입
        next_i = i + 1
        if next_i >= n or rows[next_i]["dt"] > end_dt:
            continue
...
def calc_stats(trades: list[dict], cal_days: int) -> dict:
    ...
    pf    = sum_w / sum_l if sum_l > 0 else (99.0 if sum_w > 0 else 0.0)
    wr    = len(wins) / n
...
def run_symbol(symbol: str, folds: list[dict]) -> dict:
    ...
    print(f"\n[{symbol}]")
    print(f"  {'Fold':>4}  {'IS 기간':>23}  {'IS score':>9}  {'OOS 기간':>23}  {'OOS score':>10}  {'IS tr':>5}  {'OOS tr':>6}")
    print(f"  {'':->100}")
    ...
    for fd in folds:
        ...
        is_p  = f"{fd['is_start'].strftime('%Y-%m')} ~ {fd['is_end'].strftime('%Y-%m')}"
        oos_p = f"{fd['oos_start'].strftime('%Y-%m')} ~ {fd['oos_end'].strftime('%Y-%m')}"
        print(f"  {fd['fold']:>6}  {is_p:>23}  {is_r['score']:>9.4f}  {oos_p:>23}  {oos_r['score']:>10.4f}"
              f"  {is_r['total_trades']:>5}  {oos_r['total_trades']:>6}")
    ...
    for fd in folds:
        for trade in fold_results[folds.index(fd)]["oos"].get("_trades", []):
            all_oos_trades.append(trade)

    # EV는 fold별 oos 결과의 ev_atr 가중 평균으로 대체
    oos_ev_vals = [fold_results[i]["oos"]["ev_atr"] for i in range(WF_TOTAL_FOLDS)
                   if fold_results[i]["oos"]["total_trades"] > 0]
    oos_ev_mean = sum(oos_ev_vals) / len(oos_ev_vals) if oos_ev_vals else 0.0

    # 판정
    wf_pass = wf_eff >= 0.70
    ev_pass  = oos_ev_mean > 0.0
    verdict  = "PASS" if (wf_pass and ev_pass) else "FAIL"

    fail_reason = ""
    if not wf_pass:
        fail_reason = f"WF 효율 {wf_eff:.2f} < 0.70 기준 미달"
    elif not ev_pass:
        fail_reason = f"OOS EV_ATR = {oos_ev_mean:.4f} <= 0 (수익성 부재)"
    ...
    return {
        ...
        "oos_ev_mean":  round(oos_ev_mean, 4),
        "verdict":      verdict,
...
def main() -> None:
    print("Walk-Forward Validation - Module B Long (Alt Symbols)")
    print(f"  Params: initial_sl={SL_MULT}xATR  chandelier={CHANDELIER_MULT}xATR  max_hold={MAX_HOLD_BARS}bars")
    print(f"  Criteria: WF_eff >= 0.70 AND OOS EV > 0  |  MDD limit (alt): {MDD_LIMIT_ALT*100:.0f}%")

    folds = build_folds()
    results = {}

    for sym in SYMBOLS:
        results[sym] = run_symbol(sym, folds)
    ...
    for sym, r in results.items():
        print(f"심볼: {sym}")
        print(f"IS 평균: {r['mean_is']:.4f} / OOS 평균: {r['mean_oos']:.4f}")
        print(f"WF 효율: {r['wf_efficiency']:.2f}")
        print(f"판정: {r['verdict']}")
        if r["verdict"] == "FAIL":
            print(f"원인: {r['fail_reason']}")
        print()
    print("=" * 62)
    ...
    out_path = RESULT_DIR / f"wf_mb_long_alts_{ts_str}.json"

    output = {
        "task":    "WF-MODULE-B-LONG-ALTS",
