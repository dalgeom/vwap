"""
TASK-SIMPLE-004: Condition 1 단독 P&L -- SL 구조 재조정
entry : close < VWAP_daily - 1.0 x ATR_14_1h  (신호 캔들 close 진입)
TP    : close >= VWAP_daily
SL    : close <= entry_price - 0.75 x ATR_entry
...
SL_SIGMA       = 0.75  # SL = entry - 0.75 x ATR
TIMEOUT_BARS   = 8
...
def main() -> None:
    ...
    out_path = RESULT_DIR / f"simple_cond1_sl075_{ts}.json"
    ...
    print()
    print("=" * 62)
    print("TASK-SIMPLE-004: Condition 1 P&L -- SL=0.75xATR")
