"""
TASK-BT-014: BB 스퀴즈 + OI 확인 돌파 그리드 백테스트 (결정 #47)

그리드 36케이스 = 3×2×3×2
  squeeze_thresh    : [0.70, 0.75, 0.80]   BB 수축 임계값 (SMA(BB_width,50) 대비)
  min_squeeze_bars  : [3, 5]               연속 수축 최소 봉 수
  oi_mult           : [1.10, 1.15, 1.20]   OI 급증 배수
  vwap_filter       : [True, False]        VWAP 방향 필터

심볼  : BTCUSDT / SOLUSDT / AVAXUSDT / LINKUSDT
기간  : 2023-01-01 ~ 2026-03-31
타임프레임: 1H
OI   : Bybit perp OI, SMA(OI,20봉) 기준선
진입  : 스퀴즈 해소 후 2봉 이내, close > BB_upper (LONG) 또는 < BB_lower (SHORT),
        OI 급증, VWAP 방향 일치 (filter=True 시)
청산  : Chandelier 3.0×ATR / max_hold 72봉
SL   : entry ± 1.5×ATR(1H,14)
수수료: 왕복 0.15% (0.075% × 2)
"""
...
def precompute(symbol: str) -> dict:
    ...
    oi_sma            = calc_sma(oi, OI_SMA_PERIOD)
...
def run_case(sd: dict, squeeze_thresh: float, min_squeeze_bars: int,
             oi_mult: float, vwap_filter: bool) -> dict:
    rows    = sd["rows"]
    n       = sd["n"]
    atr     = sd["atr"]
    bb_u    = sd["bb_u"]
    ...
    # oi_surge: OI > SMA(OI,20)×oi_mult AND OI > OI_prev
    oi_surge = [False] * n
    for i in range(1, n):
        o = oi[i]; os = oi_sma[i]; op = oi[i-1]
        if o is not None and os is not None and op is not None and os > 0:
            oi_surge[i] = (o > os * oi_mult) and (o > op)
    ...
    init_sl     = 0.0
    trail_sl    = 0.0
    extreme_px  = 0.0  # hi_hi (LONG) 또는 lo_lo (SHORT)
    e_dt: Optional[datetime] = None
    first_dt = last_dt = None

    # 스퀴즈 해소 이후 경과 봉 수 (>2이면 진입 불가)
...
def main() -> None:
    ...
    n_cases = (len(SQUEEZE_THRESH_LIST) * len(MIN_SQUEEZE_BARS_LIST)
               * len(OI_MULT_LIST) * len(VWAP_FILTER_LIST))

    print("TASK-BT-014: BB 스퀴즈 + OI 확인 돌파 그리드 백테스트")
    ...
    grid = list(product(
        SQUEEZE_THRESH_LIST, MIN_SQUEEZE_BARS_LIST,
        OI_MULT_LIST, VWAP_FILTER_LIST,
    ))
    print(f"[그리드 백테스트 - {len(grid)}케이스 x {len(SYMBOLS)}심볼]")

    case_results: list[dict] = []
    for idx, (sq_th, min_sq, oi_m, vf) in enumerate(grid):
        sym_stats: dict[str, dict] = {}
        for sym in SYMBOLS:
            r = run_case(sym_data[sym], sq_th, min_sq, oi_m, vf)
    ...
    def print_group(label: str, cases: list[dict]) -> None:
        top = top3(cases)
        print(f"[{label} - 상위 3개]")
        hdr = f"{'순위':^4} {'squeeze_thresh':^14} {'min_bars':^8} {'oi_mult':^7}  {'EV':^9}  {'PF':^7}  {'건/일(합산)':^10}"
        print(hdr)
        for rank, c in enumerate(top, 1):
            ...
            print(f"  {rank}     {p['squeeze_thresh']:.2f}          "
                  f"{p['min_squeeze_bars']:^8}   {p['oi_mult']:.2f}    "
    ...
    print(f"  (sq_thresh={bp['squeeze_thresh']}  min_sq={bp['min_squeeze_bars']}"
          f"  oi_mult={bp['oi_mult']}  vwap_filter={bp['vwap_filter']})")
    ...
    print("[종합]")
    print(f"VWAP 포함 최고 합산 건/일: {best_on:.4f}")
    print(f"VWAP 제외 최고 합산 건/일: {best_off:.4f}")
    print(f"타겟(0.35건/일) 달성: "
          f"{'YES' if best_on  >= 0.35 else 'NO'} (VWAP포함) / "
          f"{'YES' if best_off >= 0.35 else 'NO'} (VWAP제외)")

    # ── JSON 저장 ─────────────────────────────────────────────────────────────

    # 최우수 케이스 trades 별도 수집 (JSON 기록용)
    best_trades: dict[str, list] = {}
    for sym in SYMBOLS:
        full = run_case(sym_data[sym], bp["squeeze_thresh"],
                        bp["min_squeeze_bars"], bp["oi_mult"], bp["vwap_filter"])
        best_trades[sym] = full["trades"]

    out_path = RESULT_DIR / f"bt014_bb_squeeze_{ts_str}.json"
    output = {
        "task":         "BT-014",
        "run_at":       now.strftime("%Y-%m-%dT%H:%M:%SZ"),
        "strategy":     "BB 스퀴즈 + OI 확인 돌파 (Long/Short)",
        "period":       {"start": str(START_DT.date()), "end": str(END_DT.date())},
        "fixed_params": {
            "bb_period": BB_PERIOD, "bb_std": BB_STD,
            "bb_width_sma_period": BB_WIDTH_SMA,
            "atr_period": ATR_PERIOD,
            "oi_sma_period": OI_SMA_PERIOD,
            "sl_mult": SL_MULT,
        ...
        "summary": {
            ...
            "target_met_on":       best_on  >= 0.35,
            "target_met_off":      best_off >= 0.35,
        },
        "best_case_params":    bp,
