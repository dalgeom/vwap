"""
TASK-MB-004: Module B Long -- Cond1+2+3 동시 성립 빈도 확인
  Cond 1: close > VWAP_daily  AND  EMA9_1h > EMA20_1h
  Cond 2: abs(close - EMA9_1h) <= 0.5 x ATR_14_1h
  Cond 3: close > open  (양봉 -- 반전 확인)

mb_cond2_freq.py 에 Cond3 추가. 지표 계산 방식 동일.
룩어헤드 없음 -- 바 단위 순회.
"""
...
def analyze(symbol: str) -> dict:
    ...
    yr: dict[str, dict] = {k: {"c123": 0} for k in YEAR_RANGES}
    ...
    c123_daily_avg = c123_total / cal_days if cal_days > 0 else 0.0
    filter_rate    = (1 - c123_total / c12_total) * 100 if c12_total > 0 else 0.0

    by_year: dict[str, float] = {}
    for yr_key, (yr_s, yr_e) in YEAR_RANGES.items():
        yr_cal = (min(yr_e, RANGE_END).date() - max(yr_s, RANGE_START).date()).days + 1
        by_year[yr_key] = round(yr[yr_key]["c123"] / max(yr_cal, 1), 3)

    if c123_daily_avg >= 2:
        pf = "PASS"
    elif c123_daily_avg >= 1:
        pf = "WARN"
    else:
        pf = "FAIL"
    ...
    return {
        ...
        "cond123_total":         c123_total,
        "cond123_daily_avg":     round(c123_daily_avg, 3),
        "cond3_filter_rate_pct": round(filter_rate, 2),
        "by_year":               by_year,
        "pass_fail":             pf,
        "_cal_days":             cal_days,
    }
...
def main() -> None:
    ...
    out_path = RESULT_DIR / f"mb_cond3_freq_{ts}.json"
    ...
    print()
    print("=" * 65)
    print("TASK-MB-004: Module B  Cond1+2+3 freq check")
    print(f"  Cond3: close > open  (bullish candle confirmation)")
    ...
    for sym, r in sym_results.items():
        ...
        print(f"  Cond1+2+3 total       : {r['cond123_total']:,} bars  (daily avg {r['cond123_daily_avg']})")
        print(f"  Cond3 filter rate     : {r['cond3_filter_rate_pct']}%")
        print(f"  by_year (C1+2+3 avg)  : ", end="")
    ...
    output = {
        ...
        "verdict": verdict,
        "note": (
            f"Cond3(close>open) 추가. "
            f"verdict BTCUSDT 기준. "
    ...
    if fail_symbols:
        print()
        print("=" * 65)
        print("FAIL -- script halted. report to chairman immediately.")
