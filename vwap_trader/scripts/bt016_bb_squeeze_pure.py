"""
TASK-BT-016: BB 스퀴즈 단독 EV 검증 (결정 #49)

OI 조건 / VWAP 필터 완전 제거 - 전략 존폐 최종 기준
그리드 6케이스 = 3x2
  squeeze_thresh    : [0.70, 0.75, 0.80]
...
제외  : ATR 3배 초과 캔들 / 기존 포지션 / 신호 만료(2봉 이상)
수수료: 왕복 0.15%
"""
...
BB_WIDTH_SMA    = 50
ATR_PERIOD      = 14
ATR_SPIKE_MULT  = 3.0   # ATR 3배 초과 캔들 진입 제외
SL_MULT         = 1.5
CHANDELIER_MULT = 3.0
MAX_HOLD_BARS   = 72
...
def run_case(sd: dict, squeeze_thresh: float, min_squeeze_bars: int) -> dict:
    rows    = sd["rows"]
    n       = sd["n"]
    atr     = sd["atr"]
    bb_u    = sd["bb_u"]
...
def main() -> None:
    ...
    print("TASK-BT-016: BB 스퀴즈 단독 EV 검증 (결정 #49)")
    print(f"기간: {START_DT.date()} ~ {END_DT.date()}")
    print(f"OI 조건: 없음 / VWAP 필터: 없음")
    ...
    grid = list(product(SQUEEZE_THRESH_LIST, MIN_SQUEEZE_BARS_LIST))
    print(f"[그리드 백테스트 - {len(grid)}케이스 x {len(SYMBOLS)}심볼]")

    case_results: list[dict] = []
    for idx, (sq_th, min_sq) in enumerate(grid):
        sym_stats: dict[str, dict] = {}
        for sym in SYMBOLS:
            r = run_case(sym_data[sym], sq_th, min_sq)
        ...
        print(f"  {idx+1}/{len(grid)} sq={sq_th} min={min_sq} -> daily={comb['combined_daily']:.4f} ev={comb['ev']*100:+.3f}%")

    print()

    # ── 상위 3개 ──────────────────────────────────────────────────────────────

    sorted_cases = sorted(case_results, key=lambda c: c["combined_daily"], reverse=True)
    top3 = [c for c in sorted_cases if c["ev"] > 0][:3]

    print("[그리드 결과 - 상위 3개]")
    print(f"  {'순위':^4} {'squeeze_thresh':^14} {'min_bars':^8}  {'EV':^10}  {'PF':^7}  {'승률':^6}  {'건/일(합산)':^10}")
    for rank, c in enumerate(top3, 1):
        ...
        elig = c["eligible"]
        if elig:
            tot_n = sum(c["sym_stats"][s]["n"] for s in elig)
            wr_w  = (sum(c["sym_stats"][s]["n"] * c["sym_stats"][s]["wr"] for s in elig) / tot_n
                     if tot_n > 0 else 0.0)
        else:
            wr_w = 0.0
        print(f"  {rank}      {p['squeeze_thresh']:.2f}            "
              f"{p['min_squeeze_bars']:^8}  "
              f"{sign}{ev*100:.3f}%   {c['pf']:.3f}  {wr_w*100:.1f}%    {c['combined_daily']:.4f}")
    if not top3:
    ...
    print(f"  {'심볼':^12} {'건/일':^7} {'EV':^10} {'PF':^7} {'승률':^6} {'편입여부'}")
    ...
    combined_daily  = best_case["combined_daily"]
    strategy_alive  = ev_positive_count > 0

    print("[종합]")
    print(f"편입 심볼 합산 건/일: {combined_daily:.4f}  "
          f"(편입: {', '.join(eligible_syms) if eligible_syms else '없음'})")
    print(f"EV 양수 심볼 수: {ev_positive_count}/4")
    verdict = "생존 (EV>0 심볼 존재)" if strategy_alive else "폐기 (전 심볼 EV<=0)"
    print(f"전략 판정: {verdict}")
    print(f"타겟(0.35건/일) 달성: {'YES' if combined_daily >= 0.35 else 'NO'}")

    # ── JSON 저장 ─────────────────────────────────────────────────────────────

    best_trades: dict[str, list] = {}
    for sym in SYMBOLS:
        full = run_case(sym_data[sym], bp["squeeze_thresh"], bp["min_squeeze_bars"])
        best_trades[sym] = full["trades"]

    out_path = RESULT_DIR / f"bt016_bb_squeeze_pure_{ts_str}.json"
    output = {
        "task":         "BT-016",
        "run_at":       now.strftime("%Y-%m-%dT%H:%M:%SZ"),
        "strategy":     "BB 스퀴즈 단독 (OI/VWAP 제거)",
        "decision":     "#49",
